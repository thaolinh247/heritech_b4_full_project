...
> **16/08: chốt `PLAN-MOVEMENT-FINAL.md` + CODE XONG leg-based** — 5 chặng thao tác rời rạc (rẽ 90° tại ngã ba) thay vì bám line liên tục: `route_config.h` (5 node) + `maneuver_nav.h` (LegExecutor) + `turnLeft90/Right90/isLineCentered`; robot dừng hẳn mỗi node, chỉ rời khi app gửi NODE_DONE/NEXT_NODE/VOICE_NEXT; Finish → ALL_DONE. Build sạch PlatformIO. Còn CHỜ TEST TUYẾN THẬT (17/08 sáng) + đo thời gian rẽ; fallback nếu kẹt: quay về mode cũ (commit `4ce2bd4`).
- [ ] Chạy test tuyến thật từng chặng LEG1..5 (theo `PLAN-MOVEMENT-FINAL.md` §4): rẽ đúng ngã ba, dừng đúng node, `NODE_START:1..4`, `ALL_DONE` tại Finish; đo % rẽ đúng 90° và % dừng đúng node (cả 2 mục tiêu 100%).
- [x] Hiệu chỉnh phát hiện ngã ba (`readJunctionType()`) để gửi `WARN:turn_l/r` đúng, không báo trùng — **CODE XONG 16/08** (`main.cpp` `checkJunction()`: xác nhận 3 lần đọc liên tiếp + latch/rearm 500ms, gửi đúng 1 lần, chỉ khi `FOLLOW_LINE`; log `[JUNC]`). Chạy SONG SONG với leg-based (chỉ báo UI, không điều khiển rẽ thật). Dự phòng Phase 2: custom theo `WRO2026_B3_LineFollowing_Turns.md` nếu thư viện nhiễu.
